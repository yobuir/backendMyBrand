...
const comment_index = (req,res) =>{
 Comment.find().then((result) =>{
        ...
        return  res.send({ message: "Error while viewing comment",error: err.message })
    });
}
...
const comment_create= (req,res) =>{
    ...
    newComment.save().then((result) =>{ 
        ...
        return  res.send({ message: "Error sending comment",error: err.message })
    });
...
const comment_view= (req,res) => {
  const id=req.params.id;
       Comment.find({post_id:id,},{_id:0,__v:0}).then((result) => {
            ...
            return  res.send({ message: "Error viewing comment",error: err.message })
        });
}
